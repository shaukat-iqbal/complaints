...
import { getAllResolvedComplaints } from "../../../services/complaintService";

class resolvedChart extends Component {
  state = {
    chartData: {
      labels: [
        "January",
  ...
  async componentDidMount() {
    const { data: complaints } = await getAllResolvedComplaints();
      ...
      <div className="card bg-light border border-dark">
        <div className="container mt-3">
          {this.state.complaints.length === 0 && (
            <h3>There are no Resolved Complaints</h3>
          )}
          {this.state.complaints.length > 0 &&
